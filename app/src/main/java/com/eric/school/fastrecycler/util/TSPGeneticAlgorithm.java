...
public class TSPGeneticAlgorithm {
    private static final double DEFAULT_CROSSOVER_PROBABILITY = 0.9f; // 默认交叉概率
    private static final double DEFAULT_MUTATION_PROBABILITY = 0.01f; // 默认突变概率
    private static final int DEFAULT_POPULATION_SIZE = 30; // 默认种群数量

    private double crossoverProbability = DEFAULT_CROSSOVER_PROBABILITY; // 交叉概率
    private double mutationProbability = DEFAULT_MUTATION_PROBABILITY; // 突变概率
    ...
    public static TSPGeneticAlgorithm getInstance() {
        return GeneticAlgorithmHolder.instance;
    }

    private static class GeneticAlgorithmHolder {
        private static TSPGeneticAlgorithm instance = new TSPGeneticAlgorithm();
    }
    ...
    private void setRoulette() {
        ...
        for (int i = 0; i < values.length; i++) {
            fitnessValues[i] = 1.0 / values[i]; // 适应度为路径长的导数
        }
